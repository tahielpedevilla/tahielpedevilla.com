...
export default function Project({ project }) {
	const { title, link, mobileImage, desktopImage, agency, date } = project
	...
	return (
		...
		>
			...
			/>
			<header className="flex flex-col sm:flex-row w-full bg-[#ffffff] text-[#292929] rounded-[6px] p-4 sm:max-w-fit shadow-sm relative z-10">
				<h3 className="font-bold text-2xl tracking-tight">{title}</h3>
				<div className="flex flex-row items-center sm:ml-6 gap-2">
					<p className="text-sm lg:text-lg">{agency}</p>
					<span>/</span>
