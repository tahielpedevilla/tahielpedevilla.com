export default function AvailavilityMarquee() {
	const availavilityDate = "MARCH 2024";

	return (
		<div className="absolute bottom-6 z-20 flex items-center overflow-x-hidden max-w-[100%]">
			<div className="flex ml-6 whitespace-nowrap items-center transform marquee text-sm lg:text-lg flex-row justify-center gap-6">
				<div className="flex gap-6 items-center">
					<p className="asterisk">
						<svg
							className="w-4 h-4 fill-current"
							viewBox="0 0 75 76"
							fill="none"
							xmlns="http://www.w3.org/2000/svg"
						>
							<path d="M27.668 75.939V64.59l4.073-16.005-12.513 11.64-9.894 5.529L.022 49.167l9.603-5.529L25.921 38.4 9.625 33.453.022 27.924l9.312-16.587 9.894 5.529 12.513 11.64-4.073-16.296V.861h19.206V12.21L42.8 28.506l12.513-11.64 9.894-5.529 9.312 16.587-9.604 5.529L48.62 38.4l16.296 5.238 9.604 5.529-9.313 16.587-9.894-5.529L42.8 48.585l4.074 16.005v11.349H27.666Z" />
						</svg>
					</p>
					<p>AVAILABLE FOR FREELANCE STARTING {availavilityDate}</p>
				</div>
				<div className="flex gap-6 items-center">
					<p className="asterisk">
						<svg
							className="w-4 h-4 fill-current"
							viewBox="0 0 75 76"
							fill="none"
							xmlns="http://www.w3.org/2000/svg"
						>
							<path d="M27.668 75.939V64.59l4.073-16.005-12.513 11.64-9.894 5.529L.022 49.167l9.603-5.529L25.921 38.4 9.625 33.453.022 27.924l9.312-16.587 9.894 5.529 12.513 11.64-4.073-16.296V.861h19.206V12.21L42.8 28.506l12.513-11.64 9.894-5.529 9.312 16.587-9.604 5.529L48.62 38.4l16.296 5.238 9.604 5.529-9.313 16.587-9.894-5.529L42.8 48.585l4.074 16.005v11.349H27.666Z" />
						</svg>
					</p>
					<p>AVAILABLE FOR FREELANCE STARTING {availavilityDate}</p>
				</div>
				<div className="flex gap-6 items-center">
					<p className="asterisk">
						<svg
							className="w-4 h-4 fill-current"
							viewBox="0 0 75 76"
							fill="none"
							xmlns="http://www.w3.org/2000/svg"
						>
							<path d="M27.668 75.939V64.59l4.073-16.005-12.513 11.64-9.894 5.529L.022 49.167l9.603-5.529L25.921 38.4 9.625 33.453.022 27.924l9.312-16.587 9.894 5.529 12.513 11.64-4.073-16.296V.861h19.206V12.21L42.8 28.506l12.513-11.64 9.894-5.529 9.312 16.587-9.604 5.529L48.62 38.4l16.296 5.238 9.604 5.529-9.313 16.587-9.894-5.529L42.8 48.585l4.074 16.005v11.349H27.666Z" />
						</svg>
					</p>
					<p>AVAILABLE FOR FREELANCE STARTING {availavilityDate}</p>
				</div>
				<div className="flex gap-6 items-center">
					<p className="asterisk">
						<svg
							className="w-4 h-4 fill-current"
							viewBox="0 0 75 76"
							fill="none"
							xmlns="http://www.w3.org/2000/svg"
						>
							<path d="M27.668 75.939V64.59l4.073-16.005-12.513 11.64-9.894 5.529L.022 49.167l9.603-5.529L25.921 38.4 9.625 33.453.022 27.924l9.312-16.587 9.894 5.529 12.513 11.64-4.073-16.296V.861h19.206V12.21L42.8 28.506l12.513-11.64 9.894-5.529 9.312 16.587-9.604 5.529L48.62 38.4l16.296 5.238 9.604 5.529-9.313 16.587-9.894-5.529L42.8 48.585l4.074 16.005v11.349H27.666Z" />
						</svg>
					</p>
					<p>AVAILABLE FOR FREELANCE STARTING {availavilityDate}</p>
				</div>
				<div className="flex gap-6 items-center">
					<p className="asterisk">
						<svg
							className="w-4 h-4 fill-current"
							viewBox="0 0 75 76"
							fill="none"
							xmlns="http://www.w3.org/2000/svg"
						>
							<path d="M27.668 75.939V64.59l4.073-16.005-12.513 11.64-9.894 5.529L.022 49.167l9.603-5.529L25.921 38.4 9.625 33.453.022 27.924l9.312-16.587 9.894 5.529 12.513 11.64-4.073-16.296V.861h19.206V12.21L42.8 28.506l12.513-11.64 9.894-5.529 9.312 16.587-9.604 5.529L48.62 38.4l16.296 5.238 9.604 5.529-9.313 16.587-9.894-5.529L42.8 48.585l4.074 16.005v11.349H27.666Z" />
						</svg>
					</p>
					<p>AVAILABLE FOR FREELANCE STARTING {availavilityDate}</p>
				</div>
				<div className="flex gap-6 items-center">
					<p className="asterisk">
						<svg
							className="w-4 h-4 fill-current"
							viewBox="0 0 75 76"
							fill="none"
							xmlns="http://www.w3.org/2000/svg"
						>
							<path d="M27.668 75.939V64.59l4.073-16.005-12.513 11.64-9.894 5.529L.022 49.167l9.603-5.529L25.921 38.4 9.625 33.453.022 27.924l9.312-16.587 9.894 5.529 12.513 11.64-4.073-16.296V.861h19.206V12.21L42.8 28.506l12.513-11.64 9.894-5.529 9.312 16.587-9.604 5.529L48.62 38.4l16.296 5.238 9.604 5.529-9.313 16.587-9.894-5.529L42.8 48.585l4.074 16.005v11.349H27.666Z" />
						</svg>
					</p>
					<p>AVAILABLE FOR FREELANCE STARTING {availavilityDate}</p>
				</div>
			</div>
			<div className="flex ml-6 whitespace-nowrap items-center transform marquee text-sm lg:text-lg flex-row justify-center gap-6">
				<div className="flex gap-6 items-center">
					<p className="asterisk">
						<svg
							className="w-4 h-4 fill-current"
							viewBox="0 0 75 76"
							fill="none"
							xmlns="http://www.w3.org/2000/svg"
						>
							<path d="M27.668 75.939V64.59l4.073-16.005-12.513 11.64-9.894 5.529L.022 49.167l9.603-5.529L25.921 38.4 9.625 33.453.022 27.924l9.312-16.587 9.894 5.529 12.513 11.64-4.073-16.296V.861h19.206V12.21L42.8 28.506l12.513-11.64 9.894-5.529 9.312 16.587-9.604 5.529L48.62 38.4l16.296 5.238 9.604 5.529-9.313 16.587-9.894-5.529L42.8 48.585l4.074 16.005v11.349H27.666Z" />
						</svg>
					</p>
					<p>AVAILABLE FOR FREELANCE STARTING {availavilityDate}</p>
				</div>
				<div className="flex gap-6 items-center">
					<p className="asterisk">
						<svg
							className="w-4 h-4 fill-current"
							viewBox="0 0 75 76"
							fill="none"
							xmlns="http://www.w3.org/2000/svg"
						>
							<path d="M27.668 75.939V64.59l4.073-16.005-12.513 11.64-9.894 5.529L.022 49.167l9.603-5.529L25.921 38.4 9.625 33.453.022 27.924l9.312-16.587 9.894 5.529 12.513 11.64-4.073-16.296V.861h19.206V12.21L42.8 28.506l12.513-11.64 9.894-5.529 9.312 16.587-9.604 5.529L48.62 38.4l16.296 5.238 9.604 5.529-9.313 16.587-9.894-5.529L42.8 48.585l4.074 16.005v11.349H27.666Z" />
						</svg>
					</p>
					<p>AVAILABLE FOR FREELANCE STARTING {availavilityDate}</p>
				</div>
				<div className="flex gap-6 items-center">
					<p className="asterisk">
						<svg
							className="w-4 h-4 fill-current"
							viewBox="0 0 75 76"
							fill="none"
							xmlns="http://www.w3.org/2000/svg"
						>
							<path d="M27.668 75.939V64.59l4.073-16.005-12.513 11.64-9.894 5.529L.022 49.167l9.603-5.529L25.921 38.4 9.625 33.453.022 27.924l9.312-16.587 9.894 5.529 12.513 11.64-4.073-16.296V.861h19.206V12.21L42.8 28.506l12.513-11.64 9.894-5.529 9.312 16.587-9.604 5.529L48.62 38.4l16.296 5.238 9.604 5.529-9.313 16.587-9.894-5.529L42.8 48.585l4.074 16.005v11.349H27.666Z" />
						</svg>
					</p>
					<p>AVAILABLE FOR FREELANCE STARTING {availavilityDate}</p>
				</div>
				<div className="flex gap-6 items-center">
					<p className="asterisk">
						<svg
							className="w-4 h-4 fill-current"
							viewBox="0 0 75 76"
							fill="none"
							xmlns="http://www.w3.org/2000/svg"
						>
							<path d="M27.668 75.939V64.59l4.073-16.005-12.513 11.64-9.894 5.529L.022 49.167l9.603-5.529L25.921 38.4 9.625 33.453.022 27.924l9.312-16.587 9.894 5.529 12.513 11.64-4.073-16.296V.861h19.206V12.21L42.8 28.506l12.513-11.64 9.894-5.529 9.312 16.587-9.604 5.529L48.62 38.4l16.296 5.238 9.604 5.529-9.313 16.587-9.894-5.529L42.8 48.585l4.074 16.005v11.349H27.666Z" />
						</svg>
					</p>
					<p>AVAILABLE FOR FREELANCE STARTING {availavilityDate}</p>
				</div>
				<div className="flex gap-6 items-center">
					<p className="asterisk">
						<svg
							className="w-4 h-4 fill-current"
							viewBox="0 0 75 76"
							fill="none"
							xmlns="http://www.w3.org/2000/svg"
						>
							<path d="M27.668 75.939V64.59l4.073-16.005-12.513 11.64-9.894 5.529L.022 49.167l9.603-5.529L25.921 38.4 9.625 33.453.022 27.924l9.312-16.587 9.894 5.529 12.513 11.64-4.073-16.296V.861h19.206V12.21L42.8 28.506l12.513-11.64 9.894-5.529 9.312 16.587-9.604 5.529L48.62 38.4l16.296 5.238 9.604 5.529-9.313 16.587-9.894-5.529L42.8 48.585l4.074 16.005v11.349H27.666Z" />
						</svg>
					</p>
					<p>AVAILABLE FOR FREELANCE STARTING {availavilityDate}</p>
				</div>
				<div className="flex gap-6 items-center">
					<p className="asterisk">
						<svg
							className="w-4 h-4 fill-current"
							viewBox="0 0 75 76"
							fill="none"
							xmlns="http://www.w3.org/2000/svg"
						>
							<path d="M27.668 75.939V64.59l4.073-16.005-12.513 11.64-9.894 5.529L.022 49.167l9.603-5.529L25.921 38.4 9.625 33.453.022 27.924l9.312-16.587 9.894 5.529 12.513 11.64-4.073-16.296V.861h19.206V12.21L42.8 28.506l12.513-11.64 9.894-5.529 9.312 16.587-9.604 5.529L48.62 38.4l16.296 5.238 9.604 5.529-9.313 16.587-9.894-5.529L42.8 48.585l4.074 16.005v11.349H27.666Z" />
						</svg>
					</p>
					<p>AVAILABLE FOR FREELANCE STARTING {availavilityDate}</p>
				</div>
			</div>
		</div>
	);
}
